 ...
 * @author Compound (modified by 0xEntropy)
 */
contract ComptrollerG7 is ComptrollerV7Storage, ComptrollerInterface, ComptrollerErrorReporter, Exponential {
    ...
    /// @notice Emitted when governor is changed
    ...
    /**
     * @notice Admin function to change the governor
     * @param newGovernor The address of the new governor
     */
    function _setGovernor(address newGovernor) external {
        require(msg.sender == admin, "only admin can set governor");

        // Save current value for inclusion in log
        address oldGovernor = governor;

        // Store governor with value newGovernor
        governor = newGovernor;

        // Emit NewGovernor(OldGovernor, NewGovernor)
        emit NewGovernor(oldGovernor, newGovernor);
    }
    ...
    /*** Comp Distribution Governor (changed from admin) ***/
    ...
    function _setCompSpeeds(address[] memory cTokens, uint[] memory speeds) public {
        require(msg.sender == governor, "only governor can set comp speeds");
